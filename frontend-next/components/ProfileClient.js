...
const EditProfilePage = ({ profile, onClose, onSubmitted }) => {
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, zIndex: 3000, overflowY: 'auto', background: '#f5f6f8', padding: '2rem 1rem' }}>
            <button onClick={onClose} style={{ position: 'fixed', top: '1rem', right: '1.5rem', zIndex: 3010, background: '#1e293b', color: '#fff', border: 'none', borderRadius: '50%', width: 38, height: 38, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={18} /></button>
            ...
            <form onSubmit={handleSubmit} style={{ maxWidth: 820, margin: '0 auto', background: '#fff', borderRadius: '16px', padding: '2rem', boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
                            ...
                            {photoPreview ? (
                                <>
                                    ...
                                    <button type="button" style={{ position: 'absolute', top: 4, right: 4, background: '#ef4444', color: '#fff', border: 'none', borderRadius: '50%', width: 24, height: 24, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12 }}
                                        onClick={e => { e.stopPropagation(); setPhotoFile(null); setPhotoPreview(profile.photo ? `${API_BASE_URL}${profile.photo}` : null); }}>✕</button>
                                </>
                            ) : (
                ...
                <div style={{ background: '#fef9c3', border: '1px solid #fde047', borderRadius: '8px', padding: '0.6rem 0.9rem', marginBottom: '1.5rem', display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
                ...
                <button type="submit" disabled={loading} style={{ width: '100%', padding: '0.85rem', background: '#0f172a', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '1rem', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }} className="bn-text">
...
export default function ProfileClient() {
    ...
    if (loading) return <div className="text-center bn-text" style={{ padding: '8rem 0', color: '#64748b' }}>লোডিং...</div>;
            ...
            <div className="container" style={{ paddingTop: '2rem', paddingBottom: '6rem' }}>
                ...
                <div className="profile-container">
                    ...
                    <div style={{ gridColumn: '1 / -1' }}>
                        {pendingEdit && (
                            <div style={{ background: '#fef9c3', border: '1px solid #fde047', borderRadius: '10px', padding: '0.75rem 1.25rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                        ...
                        {photoUpdated && (
                            <div style={{ background: '#dcfce7', border: '1px solid #86efac', borderRadius: '10px', padding: '0.75rem 1.25rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <div className="profile-sidebar">
                        <div className="card text-center profile-summary-card">
                            ...
                            <h2 className="bn-text" style={{ fontSize: '1.25rem', marginTop: '1rem', fontWeight: 700, color: '#0f172a' }}>{profile.nameEn}</h2>
                            ...
                            <div className="action-buttons" style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                <button className="btn-outline w-full bn-text" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }} onClick={() => setShowEditForm(true)}>
                                    <Edit size={16} /> প্রোফাইল সম্পাদনা
                                </button>
                                <button className="btn-outline w-full bn-text" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }} onClick={() => setShowPasswordModal(true)}>
                                    <Key size={16} /> পাসওয়ার্ড পরিবর্তন
                                </button>
                            </div>

                            <div className="profile-stats text-left mt-6 pt-4" style={{ borderTop: '1px solid #e2e8f0', marginTop: '1.5rem' }}>
                                <div className="stat-row" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                                ...
                                <div className="stat-row" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                                    <span className="text-muted bn-text">সদস্য পদ:</span>
                                ...
                                <div className="stat-row" style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    ...
                                    <span className="text-red font-semibold" style={{ color: '#ef4444' }}>{profile.bloodGroup}</span>
                                </div>
                            </div>
                        </div>

                        <div className="card mt-6" style={{ marginTop: '1.5rem' }}>
                            <h3 className="bn-text font-bold mb-4" style={{ fontSize: '1.1rem', marginBottom: '1rem' }}>প্রোফাইল সম্পূর্ণতা</h3>
                            ...
                            <div className="progress-bar" style={{ height: '8px', background: '#e2e8f0', borderRadius: '999px', overflow: 'hidden', marginBottom: '0.75rem' }}>
                                <div className="progress-fill" style={{ width: `${progressPercentage}%`, height: '100%', background: '#3b82f6', transition: 'width 0.5s ease' }}></div>
                            </div>
                            <p className="bn-text text-xs text-muted mb-4" style={{ marginBottom: '1rem' }}>{completenessCriteria.length} এর মধ্যে {completedCount} ক্ষেত্র সম্পূর্ণ</p>
                            <ul className="checklist" style={{ listStyle: 'none', padding: 0 }}>
                                {completenessCriteria.map((c, i) => (
                                    <li key={i} className="bn-text text-sm mb-2" style={{ display: 'flex', alignItems: 'center', marginBottom: '0.5rem' }}>
                                        {c.isComplete ? <CheckCircle size={16} style={{ color: '#16a34a', marginRight: '8px' }} /> : <div style={{ width: '16px', height: '16px', borderRadius: '50%', border: '2px solid #cbd5e1', marginRight: '8px' }} />}
                                        <span style={{ color: c.isComplete ? '#0f172a' : '#64748b' }}>{c.label}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>
                    ...
                    <div className="profile-content">
                        <div className="card details-card">
                            <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                                ...
                                <button className="btn-outline bn-text" style={{ fontSize: '0.875rem' }}>
                                    ডিজিটাল আইডি কার্ড
                                </button>
                            </div>
                            <div className="details-grid">
                                ...
                                <div style={{ gridColumn: '1 / -1', height: '1px', background: '#e2e8f0', margin: '1rem 0' }} />
                                <div className="detail-group" style={{ gridColumn: 'span 1' }}>
                        ...
                        <div className="card timeline-card mt-6" style={{ marginTop: '1.5rem' }}>
                            <h3 className="bn-text font-bold mb-6" style={{ fontSize: '1.25rem', marginBottom: '1.5rem' }}>কার্যকলাপের সময়রেখা</h3>
                            {profile.activities && profile.activities.length > 0 ? (
                                <ActivityLog activities={profile.activities} />
                            ) : (
                                <p className="text-center text-muted text-sm bn-text py-4" style={{ padding: '1rem 0' }}>কোন কার্যকলাপ রেকর্ড করা হয়নি</p>
                            )}
                        </div>
                    </div>
                </div>
